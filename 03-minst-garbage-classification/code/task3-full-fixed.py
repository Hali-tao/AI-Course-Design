...
def load_and_filter_weights(net):
    if os.path.exists(PRETRAINED_CKPT):
        ...
        filtered_dict = {}
        print("\n" + "-"*15 + " 预训练权重热加载日志 (全量微调) " + "-"*15)
        for k, v in param_dict.items():
            if any(x in k for x in ["moments", "global_step", "learning_rate", "momentum"]):
                continue
            if "conv1.weight" in k and net_param_shapes.get(k) == (6, 3, 5, 5):
                v_expanded = ms.ops.tile(v, (1, 3, 1, 1)) / 3.0
                filtered_dict[k] = ms.Parameter(v_expanded, name=k)
                print(f"【成功适配】{k} (6,1,5,5) -> 扩展均分适配为 3 通道 (6,3,5,5)")
                continue
            if k in net_param_shapes and v.shape == net_param_shapes[k]:
                filtered_dict[k] = v
                print(f"【成功加载】{k} -> {k}")
...
if __name__ == "__main__":
    ...
    optimizer = nn.Momentum(net.trainable_params(), learning_rate=0.002, momentum=0.9)
